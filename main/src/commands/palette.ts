...
export function buildCommands(): PaletteCommand[] {
  ...
  const cmds: PaletteCommand[] = [
    { id: 'toggle_preview', group: 'View', label: 'Toggle preview', icon: 'right_panel_open', accel: accel('toggle_preview'), run: () => layout.togglePreview() },
    { id: 'toggle_split', group: 'View', label: 'Toggle split view', icon: 'vertical_split', accel: accel('toggle_split'), run: () => { const t = splitTargetId(); if (t) layout.toggleSplit(t); } },
    { id: 'swap_panes', group: 'View', label: 'Swap panes', icon: 'swap_horiz', run: () => layout.swapPanes() },
    ...
    { id: 'new_task', group: 'Task', label: 'New task', icon: 'add', accel: accel('new_task'), run: () => { if (focusedListId) startNewTask(focusedListId); } },
    { id: 'search', group: 'Task', label: 'Search this list', icon: 'search', accel: accel('search'), run: () => { if (focusedListId) focusSearch(focusedListId); } },
  ...
  for (const l of listsInProject)
    cmds.push({ id: `go-list-${l.id}`, group: 'Go to list', label: l.name, icon: 'list', run: () => layout.selectList(l.id) });
